...
import { Move, Pen, Eraser, Type, File, Image, Download, Sparkles, BrainCircuit, Mic, Bold, Italic, Underline, Group, CheckCircle, AlertTriangle, Loader, Square } from 'lucide-react';
...
// ... (Keep your Configuration, Type Definitions, and Zustand store as is)
//——————————————————————————————————————//
// 0. CONFIGURATION
//——————————————————————————————————————//
...
const useAppStore = create<AppState>((set, get) => ({
  ...
  actions: {
    ...
    startDrawing: (point) => {
      const { currentTool, penColor, penThickness, eraserThickness } = get();
      if (currentTool !== 'pen' && currentTool !== 'eraser') return;
      const newLine: LineData = { id: `line-${Date.now()}`, tool: currentTool, points: [point.x, point.y], color: penColor, thickness: currentTool === 'pen' ? penThickness : eraserThickness };
      set((state) => ({ lines: [...state.lines, newLine] }));
    },
    draw: (point) => {
      const { currentTool, lines } = get();
      if (currentTool !== 'pen' && currentTool !== 'eraser') return;
...
// ... (Keep your Helper Components & Utils as is, I will replace the relevant ones below)
//——————————————————————————————————————//
...
const ToolOptionsPanel: React.FC = () => { const { currentTool, penColor, penThickness, eraserThickness, selectedElementId, elements } = useAppStore(); const { setPenColor, setPenThickness, setEraserThickness, updateElement } = useAppStore(s => s.actions); const selectedElement = useMemo(() => elements.find(el => el.id === selectedElementId), [elements, selectedElementId]); const showPenOptions = currentTool === 'pen'; const showEraserOptions = currentTool === 'eraser'; const showTextOptions = currentTool === 'move' && selectedElement?.type === 'text'; const isBold = selectedElement?.fontStyle?.includes('bold'); const isItalic = selectedElement?.fontStyle?.includes('italic'); const isUnderlined = selectedElement?.textDecoration === 'underline'; const handleToggleBold = () => { if (!selectedElement) return; const newStyle = isBold ? (isItalic ? 'italic' : 'normal') : (isItalic ? 'bold italic' : 'bold'); updateElement(selectedElement.id, { fontStyle: newStyle }); }; const handleToggleItalic = () => { if (!selectedElement) return; const newStyle = isItalic ? (isBold ? 'bold' : 'normal') : (isBold ? 'bold italic' : 'italic'); updateElement(selectedElement.id, { fontStyle: newStyle }); }; const handleToggleUnderline = () => { if (!selectedElement) return; const newDecoration = isUnderlined ? undefined : 'underline'; updateElement(selectedElement.id, { textDecoration: newDecoration }); }; if (!showPenOptions && !showEraserOptions && !showTextOptions) return null; return (<motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} transition={{ type: 'spring', stiffness: 300, damping: 25 }} className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 bg-white/95 backdrop-blur-xl p-3 rounded-2xl shadow-2xl"> {showPenOptions && (<> {['#1f2937', '#ef4444', '#3b82f6', '#16a34a'].map(color => (<button key={color} onClick={() => setPenColor(color)} title={color} style={{ backgroundColor: color }} className={`w-7 h-7 rounded-full border-2 transition-transform duration-150 ${penColor === color ? 'border-blue-500 scale-110' : 'border-transparent hover:scale-110'}`} />))} <div className="w-px h-6 bg-gray-200 mx-1" /> {[2, 4, 8].map(thick => (<button key={thick} onClick={() => setPenThickness(thick)} className={`p-1 rounded-md transition-colors ${penThickness === thick ? 'bg-blue-100' : 'hover:bg-gray-100'}`}><div style={{ width: thick + 6, height: thick + 6, backgroundColor: penColor }} className="rounded-full" /></button>))} </>)} {showEraserOptions && (<> {[20, 40, 60].map(thick => (<button key={thick} onClick={() => setEraserThickness(thick)} className={`p-1 rounded-md transition-colors ${eraserThickness === thick ? 'bg-blue-100' : 'hover:bg-gray-100'}`}><div style={{ width: thick / 2 + 8, height: thick / 2 + 8 }} className="rounded-full bg-gray-300 border-2 border-gray-400" /></button>))} </>)} {showTextOptions && selectedElement && (<> {['#1f2937', '#ef4444', '#3b82f6', '#16a34a'].map(color => (<button key={color} onClick={() => updateElement(selectedElement.id, { fill: color })} title={color} style={{ backgroundColor: color }} className={`w-7 h-7 rounded-full border-2 transition-transform duration-150 ${selectedElement.fill === color ? 'border-blue-500 scale-110' : 'border-transparent hover:scale-110'}`} />))} <div className="w-px h-6 bg-gray-200 mx-1" /> <button title="Bold" onClick={handleToggleBold} className={`p-2 rounded-md transition-colors ${isBold ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100 text-gray-600'}`}> <Bold size={18} /> </button> <button title="Italic" onClick={handleToggleItalic} className={`p-2 rounded-md transition-colors ${isItalic ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100 text-gray-600'}`}> <Italic size={18} /> </button> <button title="Underline" onClick={handleToggleUnderline} className={`p-2 rounded-md transition-colors ${isUnderlined ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100 text-gray-600'}`}> <Underline size={18} /> </button> </>)} </motion.div>); };
...
// ✨ --- CORRECTED RENDER-TO-IMAGE FUNCTION --- ✨
const renderMarkdownToImage = async (content: string, width: number): Promise<{ dataURL: string; height: number }> => {
  const container = document.createElement('div');
  document.body.appendChild(container);

  // Apply styles for plain text rendering
  ...
  style.innerHTML = `
    ...
    pre { background-color: #0d1117; color: #c9d1d9; padding: 16px; border-radius: 6px; overflow-x: auto; }
    pre code.hljs { display: block; overflow-x: auto; padding: 0; background: transparent; color: inherit; }
  `;
  container.appendChild(style);

  // Process math and markdown
  const processedContent = content
    .replace(/\$\$(.*?)\$\$/gs, '<div class="math-display">$$$1$$</div>')
    .replace(/\$(.*?)\$/g, '<span class="math-inline">$$$1$</span>');

  // Use insertAdjacentHTML for safer DOM manipulation
...
const TutorLM: React.FC = () => {
  ...
  const { currentTool, lines, elements, selectedElementId, editingElementId, aiState, transcript, isUploading } = useAppStore();
  ...
  // ✨ --- CORRECTED AI REQUEST HANDLER --- ✨
  const handleAIChatRequest = async (prompt: string) => {
    const cardId = `ai-card-${Date.now()}`;
    const pos = getPointerPosition();
    // 1. Add a placeholder text element that will be streamed into
    actions.addElement({ id: cardId, type: 'text', x: pos.x, y: pos.y, content: '...', fontSize: 18, fill: '#1f2937' });
    let fullResponse = '';

    try {
      const response = await fetch(`${BACKEND_URL}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt }) });
      if (!response.ok || !response.body) { throw new Error(`Network response was not ok: ${response.statusText}`); }
      ...
      while (true) {
        ...
        fullResponse += word;
        // 2. Stream the response into the text element on the canvas
        actions.streamAIResponse(cardId, word);
      }

      // 3. Once streaming is complete, render the final markdown to a transparent image
      const { dataURL, height } = await renderMarkdownToImage(fullResponse, 600);

      // 4. Update the element from a text type to an image type
      actions.updateElement(cardId, {
        type: 'image',
        content: dataURL,
        width: 600,
        height: height,
      ...
      // On error, update the text element with an error message
      actions.updateElement(cardId, { content: `Sorry, an error occurred. Please try again.` });
    } finally {
  ...
  const handleProcessDrawing = async () => {
    ...
    try {
      ...
      for (const cluster of clusters) {
        ...
        // First, capture for display with a transparent background
        const displayDataUrl = stage.toDataURL({ ...clipRect, pixelRatio: 2 });
        const displayBlob = dataURLtoBlob(displayDataUrl);

        // Temporarily add a white background for better image capture for the backend
  ...
  const handleMouseDown = (e: Konva.KonvaEventObject<MouseEvent | TouchEvent>) => { const clickedOnEmpty = e.target === e.target.getStage(); if (clickedOnEmpty) { actions.setSelectedElementId(null); actions.setEditingElementId(null); } if (currentTool === 'text' && clickedOnEmpty) { const newId = `text-${Date.now()}`; actions.addElement({ id: newId, type: 'text', x: getPointerPosition().x, y: getPointerPosition().y, content: 'Type here...', fontSize: 24, fill: '#1f2937' }); actions.setEditingElementId(newId); actions.setCurrentTool('move'); return; } if ((currentTool === 'pen' || currentTool === 'eraser') && clickedOnEmpty) { isDrawing.current = true; actions.startDrawing(getPointerPosition()); } else if (currentTool === 'move' && clickedOnEmpty) { isPanning.current = true; } };
  ...
  return (
    <div className="h-screen w-screen bg-gray-100 overflow-hidden relative font-sans" style={{ cursor: cursorStyle }}>
      ...
      <div className="absolute top-7 left-7 z-50 select-none"><h1 className="text-2xl font-bold" style={{ fontFamily: "'Satisfy', cursive" }}>TutorLM</h1></div>
      ...
      <AnimatePresence>{showWelcome && (<motion.div initial={{ opacity: 0, scale: .9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: .9, y: -20 }} className="fixed top-30 left-1/2 -translate-x-1/2 z-40 max-w-md p-6 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl"><div className="flex items-center gap-3 mb-3"><div className="p-2 bg-gradient-to-br from-blue-500 to-blue-400 rounded-lg"><Sparkles className="w-5 h-5 text-white" /></div><h3 className="font-bold text-gray-800">Welcome to TutorLM!</h3></div><p className="text-sm text-gray-600">Your AI learning companion. Upload files, draw, and talk to the AI by holding [Space].</p></motion.div>)}</AnimatePresence>
      ...
      <div className="fixed top-7 left-1/2 -translate-x-1/2 z-50">
        <AnimatePresence mode="wait">
          {aiState !== 'idle' ? (
            ...
            <motion.div key="toolbar" initial={{ opacity: 0, y: -20, scale: .95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: .95 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} className="flex items-center gap-3 bg-white/95 backdrop-blur-xl p-3 rounded-2xl shadow-2xl">
